  ...
  const renderEventCards = (events: IEvent[]) => {
    if (!isLoading) {
      if (isArray(events) && events.length > 0) {
        ...
        // filter by Date
        const filteredArray = sortedArray.filter((date) =>
          filterByActualYear(date.eventDate)
        );
        filteredArray.length = 3;

        const toogleVisibility = isAuthenticated
          ? sortedArray
          : filteredArray.filter((e) => e.hidden !== true);
