...
export class ApiServer implements IHttpServer {
	...
	public start(port?: number): void {
		...
		if (
			!existsSync(config.default.root + "/src/routes/sslcert/cert.pem") &&
			!existsSync(config.default.root + "/src/routes/sslcert/key.pem")
		) {
		...
		this._restifyServer.listen(port, () => {
			console.log(
				"\x1b[33m%s\x1b[0m",
				`[${config.default.envName}]-mode Started http server with ${cpus()
					.length} clusters on Host ${hostname} => http://localhost:${config.default
					.httpPort} for Process Id ${process.pid}`
			);
		});
		...
		this._restifyHttpsServer.listen(config.default.httpsPort, () => {
			console.log(
				"\x1b[33m%s\x1b[0m",
				`[${config.default.envName}]-mode Started https server with ${cpus()
					.length} clusters on Host ${hostname} => https://localhost:${config.default
					.httpsPort} for Process Id ${process.pid}`
			);
		});
	}
}
